...
def save_hidden_states_to_csv(hidden_states, labels, save_path='./hidden_states.csv'):
    hidden_states = hidden_states.detach().cpu().numpy()
    labels = labels.detach().cpu().numpy().reshape(-1, 1)
    data = pd.DataFrame(data=hidden_states)
    data['label'] = labels
...
class DebertaScratch(DebertaV2PreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, labels=None):
        hidden_states_flag = labels is not None
        outputs = self.deberta(input_ids=input_ids, attention_mask=attention_mask, output_hidden_states=hidden_states_flag)
...
if __name__ == '__main__':
    ...
    result_output.to_csv("./deberta_v3_base_scl_multiview.csv", index=False, quoting=3)